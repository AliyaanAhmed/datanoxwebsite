      ...
      {items.map((item, index) => (
        <Reveal as="li" key={item.name} delay={index * 80}>
          ...
          >
            ...
            {item.detail ? (
              <p className="mt-auto pt-6 text-[0.8125rem] font-semibold tracking-[0.01em] text-o-700">
                {item.detail}
              </p>
            ) : null}
          </div>
      ...
      {items.map((item, index) => (
        ...
        >
          ...
          >
            ...
            {item.detail ? (
              <p className="shrink-0 text-[0.8125rem] font-semibold tracking-[0.01em] text-o-700 lg:pt-3 lg:text-right">
                {item.detail}
              </p>
            ) : null}
          </div>
    ...
    <div className="mt-12 grid gap-5 lg:grid-cols-[1.05fr_1.35fr]">
      <Reveal>
        ...
        >
          ...
          {lead.detail ? (
            <p className="relative mt-auto pt-7 text-[0.8125rem] font-semibold tracking-[0.01em] text-white/75">
              {lead.detail}
            </p>
          ) : null}
        </div>
          ...
          >
            ...
            <div>
              <h3 className="flex flex-wrap items-baseline gap-x-3 gap-y-1 font-display text-[1.125rem] font-semibold text-ink">
                {item.name}
                {item.detail ? (
                  <span className="text-[0.75rem] font-semibold tracking-[0.01em] text-o-700">
                    {item.detail}
                  </span>
                ) : null}
              </h3>
